...
export default function SharedNotesPage() {
  ...
  const handleDelete = async (noteId) => {
    const isConfirmed = await requestDeleteConfirmation("Are you sure you want to delete this note?");
    if (isConfirmed) {
      try {
          ...
          <div className="flex flex-col lg:flex-row gap-6 justify-center">
            ...
            <div className="w-full max-w-[580px]">
              <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl bg-white p-5 shadow-sm border border-slate-200/60">
                <div>
                  <h1 className="text-xl font-bold tracking-tight text-slate-900">All Shared Notes</h1>
                  <p className="text-slate-500 text-[13px] mt-0.5 font-medium">Browse study resources shared by the community.</p>
                </div>
                <button 
              ...
              <div className="space-y-3.5">
                {isLoading && page === 1 ? (
                  ...
                  <div className="rounded-xl border border-rose-200 bg-rose-50 p-8 text-center shadow-sm">
                    <p className="font-bold text-rose-700 text-sm">Failed to load shared notes.</p>
                  </div>
                ) : notes.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
                    ...
                    <h3 className="mt-5 font-bold text-lg text-slate-900">No notes shared yet</h3>
                    <p className="mt-1 text-slate-500 font-medium text-sm">Be the first to share a helpful resource!</p>
                    <button 
                      onClick={() => setIsModalOpen(true)}
                  ...
                  <>
                    ...
                    {isFetching && (
                      Array.from({ length: 2 }).map((_, i) => (
                        <div key={`skeleton-${i}`} className="animate-pulse rounded-xl border border-slate-200 bg-white p-5 h-28 shadow-sm" />
                      ))
                    )}

                    {!hasMore && notes.length > 0 && (
                      <div className="py-8 text-center text-slate-500 font-medium text-sm">
                        You've seen all the notes for now!
                      </div>
                    )}
                  </>
                )}
