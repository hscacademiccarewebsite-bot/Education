...
export default function CommunityPage() {
  ...
  const { data: postsData, isLoading, isError } = useGetPostsQuery({ page: 1, limit: 20 }, {
        ...
        <div className="flex flex-col lg:flex-row gap-6 justify-center">
          ...
          <aside className="hidden xl:block w-[280px] space-y-4 sticky top-[100px] h-fit">
            ...
            <div className="border-t border-slate-200 pt-4 px-2">
              <h4 className="text-[17px] font-bold text-slate-500 mb-3">Contacts</h4>
              <div className="space-y-1">
                <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-200/50 transition-all cursor-pointer">
                  <div className="relative">
                    <Avatar className="h-8 w-8 rounded-full bg-slate-200" name="Staff" />
                    <div className="absolute right-0 bottom-0 h-2.5 w-2.5 rounded-full border-2 border-white bg-emerald-500" />
                  </div>
                  <span className="text-[14px] font-bold text-slate-800">HSC Support</span>
                </div>
              </div>
            </div>
          </aside>
